...
_Cilk_shared const int length = 10000000;
...
double _Cilk_shared  fit(_Cilk_shared myFunc *  functionp, _Cilk_shared double * NLLS, int scanNumber,  int threads,  _Cilk_shared double *  data,  int length){
...
int main(){
    ...
    int scanNumber = 1000;
    funcp = &gauss;
    data =  (_Cilk_shared double * ) _Offload_shared_aligned_malloc(sizeof(double)*length,64);
    gauss.setParameter(sigma);
    gauss.generateData(length,data);
    printf("Finished generating data\n");
    NLLS = (_Cilk_shared double *  ) _Offload_shared_aligned_malloc(sizeof(double)*scanNumber,64);
    FILE * output = fopen("unoffloaded.dat","w");
    for(threads=1; threads<=24; threads+=1){
        double time = omp_get_wtime();
        double minSig = _Cilk_offload fit(funcp,NLLS,scanNumber,threads,data,length);
